...
func run(context: ActionContext) -> void:
	for target in resolve_targets(context):
		if status_effect:
			var stacks_to_apply := upgrade_stacks if target.has_status_effect(status_effect) else applied_stacks
			target.add_status_effect(status_effect, context.source, stacks_to_apply, max_stacks)
